...
const apiCanaryBlueprint = async function () {
 ...
 const testIntegrityForLayer = async function(datasets, countryISOCodes, layer, operation){
  // TEST #1
  // Find sum of all Modis alerts for the most recent completed week in the WDPA weekly table
  ...
  requestOptions.path = "/v1/query/?sql=select%20sum%28alert__count%29%20as%20sum_alert_count%20from%20" +
    datasets.ModisWdpaWeekly + "%20where%20alert__year%20%3D%20" + currYear + "%20and%20and%20alert__week%3D" + currWeek +
  ...
  requestOptions.path = "/v1/query/?sql=select%20sum%28alert__count%29%20as%20sum_alert_count%20from%20" +
    datasets.ModisWdpaDaily + "%20where%20alert__date%3E%3D%27" +
  ...
  await secretsManager.getSecretValue({ SecretId: "gfw-api/datasets" }, function(err, data) {
      if (err) log.info(err, err.stack);
      log.info(data);
      datasets.ModisWdpaWhitelist = JSON.parse(data["SecretString"])["MODIS_WDPA_whitelist"];
      datasets.ModisWdpaWeekly = JSON.parse(data["SecretString"])["MODIS_WDPA_weekly"];
      datasets.ModisWdpaDaily = JSON.parse(data["SecretString"])["MODIS_WDPA_daily"];
  }).promise();
  ...
  requestOptions.path = "/v1/query/?sql=select%20*%20from%20" + datasets.ModisWdpaWhitelist + "%20where%20wdpa_protected_area__iso%20in%20%28" + countriesISOCodes + "%29";
  ...
  for (const row of response.data) {
    if (row.wdpa_protected_area__iso===null) {
        throw new Error("no entry returned from Modis WDPA Whitelist table");
    }
    else {
