...
const apiCanaryBlueprint = async function () {
 ...
 const getFormattedDate = function(incomingDate) {
  ...
  let datasets = {
      ...
      VIIRS_geostore_daily: "",
  };
  await secretsManager.getSecretValue({ SecretId: "gfw-api/datasets" }, function(err, data) {
      if (err) log.info(err, err.stack);
      log.info(data);
      datasets.VIIRS_geostore_weekly = JSON.parse(data["SecretString"])["VIIRS_geostore_weekly"];
      datasets.VIIRS_geostore_daily = JSON.parse(data["SecretString"])["VIIRS_geostore_daily"];
  }).promise();
  

  // TEST #1
  // Find sum of all VIIRS alerts for the most recent week in the geostore weekly table
  let sumVIIRSAlerts = 0;
  const currDate = new Date();
  const currYear = currDate.getFullYear();
  const currWeek = getWeek(currDate);
  requestOptions.path = "/v1/query/?sql=select%20sum%28alert__count%29%20as%20sum_alert_count%20from%20" + datasets.VIIRS_geostore_weekly + "%20where%20alert__year%20%3D%20" + currYear + "%20and%20alert__week%3D" + currWeek;
  ...
  requestOptions.path = "/v1/query/?sql=select%20sum%28alert__count%29%20as%20sum_alert_count%20from%20" + datasets.VIIRS_geostore_daily + "%20where%20alert__date%20%3E%3D%27" + getFormattedDate(lastMonday) + "%27%20and%20alert__date%3C%3D%27" + getFormattedDate(currDate) + "%27";
