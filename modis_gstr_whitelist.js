...
const apiCanaryBlueprint = async function () {
 ...
 const testIntegrityForLayer = async function(datasets, layer, operation){
  // TEST #1
  // Find sum of all Modis alerts for the most recent completed week in the geostore weekly table
  ...
  requestOptions.path = "/v1/query/?sql=select%20sum%28alert__count%29%20as%20sum_alert_count%20from%20" +
    datasets.ModisGeostoreWeekly + "%20where%20alert__year%20%3D%20" + currYear + "%20and%20and%20alert__week%3D" + currWeek +
  ...
  // TEST #2
  // Find sum of all Modis alerts for Brazil for the most recent completed week in the geostore daily table
  ...
  requestOptions.path = "/v1/query/?sql=select%20sum%28alert__count%29%20as%20sum_alert_count%20from%20" +
    datasets.ModisGeostoreDaily + "%20where%20alert__date%3E%3D%27" +
  ...
  await secretsManager.getSecretValue({ SecretId: "gfw-api/datasets" }, function(err, data) {
      if (err) log.info(err, err.stack);
      log.info(data);
      datasets.ModisGeostoreWhitelist = JSON.parse(data["SecretString"])["MODIS_geostore_whitelist"];
      datasets.ModisGeostoreWeekly = JSON.parse(data["SecretString"])["MODIS_geostore_weekly"];
      datasets.ModisGeostoreDaily = JSON.parse(data["SecretString"])["MODIS_geostore_daily"];
  }).promise();
  ...
  requestOptions.path = "/v1/query/?sql=select%20*%20from%20" + datasets.ModisGeostoreWhitelist;
  ...
  for (const row of response.data) {
    for (const [key, value] of contextualLayers){
        if (totalModisAlertsMap.get(key)===undefined && row[key]===true){
            totalModisAlerts = await testIntegrityForLayer(datasets, key, value);
