...
const apiCanaryBlueprint = async function () {
 ...
 const getFormattedDate = function(incomingDate) {
  ...
  await secretsManager.getSecretValue({ SecretId: "gfw-api/datasets" }, function(err, data) {
      if (err) log.info(err, err.stack);
      log.info(data);
      datasets.MODIS_WDPA_weekly = JSON.parse(data["SecretString"])["MODIS_WDPA_weekly"];
      datasets.MODIS_WDPA_daily = JSON.parse(data["SecretString"])["MODIS_WDPA_daily"];
  }).promise();
  

  // TEST #1
  // Find sum of all MODIS alerts for the most recent week in the WDPA weekly table
  let sumMODISAlerts = 0;
  const currDate = new Date();
  const currYear = currDate.getFullYear();
  const currWeek = getWeek(currDate);
  requestOptions.path = "/v1/query/?sql=select%20sum%28alert__count%29%20as%20sum_alert_count%20from%20" + datasets.MODIS_WDPA_weekly + "%20where%20alert__year%20%3D%20" + currYear + "%20and%20alert__week%3D" + currWeek;
  ...
  responseWDPAWeekly.data.forEach(row => {
    if (row.sum_alert_count===null) {
      throw new Error("sum of all MODIS alerts from WDPA weekly table not returned");
    }
    else if (row.sum_alert_count===0){
        throw new Error("sum of all MODIS alerts from WDPA weekly for the past week is 0");
    }
    else {
        sumMODISAlerts = row.sum_alert_count;
        log.info("Successfully returned sum of all MODIS alerts for the past week from WDPA weekly table: " + sumMODISAlerts);
    }
  });  
  
  
  // TEST #2
  // Find sum of all MODIS alerts for the most recent completed week in the WDPA daily table
  ...
  requestOptions.path = "/v1/query/?sql=select%20sum%28alert__count%29%20as%20sum_alert_count%20from%20" + datasets.MODIS_WDPA_daily + "%20where%20alert__date%20%3E%3D%27" + getFormattedDate(lastMonday) + "%27%20and%20alert__date%3C%3D%27" + getFormattedDate(currDate) + "%27";
  const responseWDPADaily = await verifyRequest(requestOptions);
  //Iterate through each of the rows of the data in the response
  responseWDPADaily.data.forEach(row => {
    if (!row.sum_alert_count) {
      throw new Error("sum of all MODIS alerts from WDPA daily table not returned");
    }
    else if (row.sum_alert_count!=sumMODISAlerts){
        throw new Error("sum of all MODIS alerts from WDPA daily for the past week is not equal to the sum of all MODIS alerts from WDPA weekly for the past week: " + sumMODISAlerts);
    }
    else {
        log.info("Successfully returned sum of all MODIS alerts for the past week from WDPA daily table: " + row.sum_alert_count);
    }
  });  
